...
import random
def addforcafe():
    print("Would you like to add another user here? (y/n)")
    yeno = input()
    condition = ["y","n"]
    if yeno == condition[0]:
        usnum = int(input("Select the number of users to be added:"))
        for x in range(0,usnum):
            ...
            billing = open("billing.csv","a",newline="")
            bw = csv.writer(billing)
            bw.writerow(neededlist)
            billing.close()

    elif yeno == condition[1]:
            pass
